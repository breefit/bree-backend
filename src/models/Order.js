...
export const appendStatusHistory = async ({ orderId, previousStatus = null, newStatus, changedBy = null, notes = null }) => {
  return query(
...
export const getStatusHistory = async (orderId) => {
  const { rows } = await query(
    `SELECT id, previous_status, new_status, changed_by, notes, created_at FROM order_status_history WHERE order_id = $1 ORDER BY created_at ASC`,
    [orderId]
  );
  return rows;
};
