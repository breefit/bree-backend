...
const errorHandler = (err, req, res, next) => {
  ...
  if (err.code === "LIMIT_FILE_SIZE") {
    return res
      .status(400)
      .json({ message: "File too large. Maximum 5 MB allowed." });
  }
  ...
  if (err.type === "entity.too.large") {
    return res.status(413).json({
      message:
        "Request payload too large. Upload an image under 5 MB using multipart/form-data.",
    });
  }
